...
async function main() {
	...
	const macUrl = assets.data.find(s => s?.name?.endsWith('.tar.gz'))?.browser_download_url

	const windowsUrl = assets.data.find(s => s?.name?.endsWith('.zip'))?.browser_download_url

	if (macUrl) {
		const macSigAsset = assets.data.find(s => s?.name?.endsWith('.gz.sig'))

		const macSigAssetId = macSigAsset?.id
		// To get at the signature asset content generated by the tauri action, we have to return it as binary and then decode it
		...
		const signature = result?.data ? utf8Decoder.decode(result.data) : undefined
